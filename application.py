...
def register():
    ...
    if request.method == "POST":
        ...
        # Store username & password into databaseㄝ then login
        try:
...
def search():
    ...
    if request.method == "POST":
        ...
        try:
            books = db.execute("SELECT * FROM books WHERE isbn LIKE :isbn AND LOWER(title) LIKE LOWER(:title) AND LOWER(author) LIKE LOWER(:author)",
...
def detail(book_id):
    ...
    if request.method == "POST":
        if session.get("username"):
            ...
            if db.execute("SELECT username FROM reviews WHERE username=:username AND book_id=:book_id",{"username":username, "book_id":book_id}).rowcount!=0:
                flash("You have already make a review.", "warning")
                return redirect(url_for('detail', book_id=book.id))

            # Save the review into database
            try:
                db.execute("INSERT INTO reviews (book_id, rating, content, username) VALUES (:book_id, :rating, :content, :username)",
...
def books_api(isbn):
    ...
    review_count = db.execute("SELECT COUNT(*) FROM reviews JOIN books ON books.id = reviews.book_id WHERE isbn = :isbn",
                              {"isbn": isbn}).fetchone()
    average_score = db.execute("SELECT AVG(rating) FROM reviews JOIN books ON books.id = reviews.book_id WHERE isbn = :isbn",
                               {"isbn": isbn}).fetchone()
    data = {
        "title": book.title,
        "author": book.author,
